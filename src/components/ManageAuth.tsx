...
const AUTH_SERVICES = ['Gmail Trigger', 'Gmail', 'Notion', 'Youtube', 'Linkedin', 'Googlecalendar', 'Googledocs', 'Googlesheets'];
...
export default function ManageAuth() {
  ...
  const updateCache = (data: { user_auths: { [key: string]: boolean }; api_keys: { [key: string]: string } }) => {
  ...
  const handleAuthToggle = async (service: string, enabled: boolean) => {
    try {
      ...
      const response = await fetch(endpoint, {
        ...
        redirect: 'manual',
        body: JSON.stringify({ service: service.toLowerCase().replace(/\s+/g, '') }),
      });
    ...
    <div className="h-screen flex flex-col items-center bg-gray-900 text-white p-6">
      ...
      <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
          <h3 className="text-xl font-semibold mb-4 text-center">Enter API Keys</h3>
          {error && <p className="text-red-500 text-sm mb-3">{error}</p>}
          <form onSubmit={handleSubmit} className="space-y-4">
            {['OpenAI', 'Gemini', 'Composio'].map((key) => (
              <div key={key} className="relative">
                <label className="block text-sm font-medium mb-1">
                  {key} API Key {key === 'Composio' && '(Required)'}
                </label>
                <div className="flex items-center bg-gray-700 rounded px-3 py-2">
                  <input
                    type={showPassword[key.toLowerCase()] ? 'text' : 'password'}
                    value={keys[key.toLowerCase()] || ''}
                    onChange={(e) => setKeys({ ...keys, [key.toLowerCase()]: e.target.value })}
                    className="flex-grow bg-transparent outline-none"
                    required={key === 'Composio'}
                  />
                  <button type="button" onClick={() => togglePasswordVisibility(key.toLowerCase())} className="ml-2 text-gray-400 hover:text-white">
                    {showPassword[key.toLowerCase()] ? '🙈' : '👁️'}
                  </button>
                  <button type="button" onClick={() => copyToClipboard(key.toLowerCase())} className="ml-2 text-gray-400 hover:text-white">
                    📋
                  </button>
                </div>
              </div>
        ...
        <div className="fixed inset-0 flex flex-col items-center justify-center bg-black bg-opacity-80 z-50">
          ...
          <p className="mt-4 text-lg">fetching auth status... Please wait</p>
        </div>
      )}
          ...
          {Object.keys(keys).length > 0 ? (
                    ...
                    <td className="p-2 text-center">
                      <label className="relative flex items-center cursor-pointer">
                        <input
                          ...
                          onChange={() => handleAuthToggle(service, !authStatus[service])}
                        ...
                        >
                          <div
                            className={`w-5 h-5 bg-white rounded-full shadow-md transform transition ${
                              authStatus[service] ? 'translate-x-6' : 'translate-x-1'
                            }`}
                          ></div>
                        </div>
                      </label>
            ...
            <p className="text-gray-400 text-center">No API keys entered yet.</p>
          )}
        </div>
      </div>
    </div>
